...
class PostgisGeomReader {
  ...
  final GeometryFactory _geomFactory = DEFAULT_GEOM_FACTORY;

  ByteOrderDataInStream _din;

  String geometryType;

  PostgisGeomReader(List<int> ins) {
    if (ins is Uint8List) {
      _dataBuffer = ins;
    } else {
  ...
  Geometry _read() {
    WKBReader wkbReader = WKBReader.withFactory(_geomFactory);
